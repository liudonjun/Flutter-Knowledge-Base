---
layout: home

hero:
  name: Flutter 知识库
  text: 系统化的 Flutter 开发学习资源
  tagline: 从入门到精通的完整学习路径
  actions:
    - theme: brand
      text: 开始学习
      link: /guide/introduction
    - theme: alt
      text: 在 GitHub 上查看
      link: https://github.com/liudonjun/Flutter-Knowledge-Base

features:
  - icon: 📚
    title: 系统化学习
    details: 14个主要模块覆盖 Flutter 全栈开发，从基础到高级的完整学习路径。
  - icon: 🎯
    title: 实战导向
    details: 理论学习与实践相结合，提供丰富的实战项目和代码示例。
  - icon: 🔗
    title: 知识网络
    details: 通过双括号维基链接建立知识关联，模块索引与专题页互相跳转，便于复习和回顾。
  - icon: 🚀
    title: 持续更新
    details: 定期更新学习内容，跟进 Flutter 最新特性和最佳实践。
---

## 🎯 快速开始

### 初学者路径
1. [Flutter 简介](/guide/introduction) - 了解 Flutter 框架
2. [环境搭建](/guide/setup) - 配置开发环境
3. [第一个应用](/guide/first-app) - 创建 Hello World

### 进阶路径
1. [Widget 系统](/guide/widgets) - 掌握 Widget 核心
2. [状态管理](/guide/state-management) - 学习状态管理方案
3. [导航与路由](/guide/navigation) - 实现页面导航

### 高级路径
1. [架构概览](/core/architecture) - 深入理解架构
2. [电商应用](/projects/ecommerce) - 完整项目实战
3. [官方资源](/resources/official) - 精选学习资源

## 📊 知识库概览

- **模块数量**: 14个主要模块
- **文章数量**: 500+ 篇知识库长文（全站 540+ Markdown）
- **学习路径**: 从入门到精通
- **实战项目**: 10+ 完整项目
- **学习资源**: 100+ 精选资源

## 📚 主要模块

### 基础模块
- [核心概念](/01-核心概念/00-核心概念索引) - Flutter 架构、渲染原理、Widget 生命周期
- [Dart 语言](/03-Dart语言/00-Dart语言索引) - 从基础语法到元编程
- [开发环境](/02-开发环境/00-开发环境索引) - 环境搭建、项目配置

### 核心技术
- [Widget 系统](/04-Widget系统/00-Widget系统索引) - Widget 全面解析
- [状态管理](/05-状态管理/00-状态管理索引) - Provider、Bloc、Riverpod、GetX
- [导航与路由](/06-导航与路由/00-导航与路由索引) - 基础路由到 GoRouter
- [网络与数据](/07-网络与数据/00-网络与数据索引) - HTTP、WebSocket、数据缓存

### 进阶主题
- [UI 与动画](/08-UI与动画/00-UI与动画索引) - 动画系统、自定义绘制
- [测试与调试](/10-测试与调试/00-测试与调试索引) - 单元测试到自动化测试
- [性能优化](/11-性能优化/00-性能优化索引) - 内存优化、渲染优化
- [项目实战](/12-项目实战/00-项目实战索引) - 代码规范、架构设计

## 🔗 相关链接

- [完整知识库（十四模块原文）](/handbook/)
- [维基链接清单](/handbook/wiki-link-backlog)（维护用，当前 backlog 为 0）

- [Flutter 官方文档](https://flutter.dev/docs)
- [Dart 官方文档](https://dart.dev/guides)
- [Flutter GitHub](https://github.com/flutter/flutter)
- [知识库 GitHub](https://github.com/liudonjun/Flutter-Knowledge-Base)